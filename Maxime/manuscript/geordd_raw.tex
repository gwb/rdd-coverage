...
Once we have obtained the posterior of \(\tau\) \autoref{eq:postvar2gp}, estimating the local average treatment effect (LATE) along the border will often be of interest.
...
We geocode the address of each sale by merging the sales with the NYC Department of Finance's Digital Tax Map, which contains X and Y coordinates for the centroid of every parcel in NYC, identified by its borough, zip code, block and lot.
...
\begin{table}
    ...
    \begin{tabular}{lrrr}
        ...
        \hline
          % \(\unifavg\) & -0.16 & 0.09 & 4.41\% \\
          % \(\taurho\) & -0.19 & 0.06 & 0.14\% \\
          % \(\invvar\) & -0.20 & 0.06 & 0.09\% \\
          % \(\tauproj\) & -0.17 & 0.08 & 1.95\% \\
          % \(\taugeo\) & -0.14 & 0.13 & 15.08\% \\
          % \(\taupop\) & -0.17 & 0.08 & 1.25\% \\
          \(\unifavg\) & -0.16 & 0.09 & 4.39\% \\
          \(\taurho\) & -0.19 & 0.06 & 0.14\% \\
          \(\invvar\) & -0.20 & 0.06 & 0.09\% \\
          \(\tauproj\) & -0.17 & 0.08 & 1.91\% \\
          \(\taugeo\) & -0.14 & 0.13 & 15.38\% \\
          \(\taupop\) & -0.17 & 0.08 & 1.24\% \\
        \hline
    \end{tabular}
    \caption{
    \label{table:NYC_ate}
...
While our framework is intuitive and well-motivated by the 1D RDD literature,
it does have drawbacks.
It does not specify a prior directly on the treatment effect along the border;
instead it can be shown that our GP model implicitly gives it a wide prior for a constant effect
plus a Gaussian process (GP) prior with double the covariance function \(k\) in \autoref{eq:spec2gp}.
...
The calibration of the \(p\)-values in \autoref{sec:hypothesis_testing} safeguards the validity of our proposed hypothesis tests
--- further validated by placebo tests ---
but RIC could bias the cliff-face and LATE estimates.
Unfortunately the proposed solution of first regressing the treatment variable on
control variables (spatial covariates in GeoRDDs) to estimate the propensity score cannot be used:
the propensity score for GeoRDDs is already known to be 0 or 1 in
the control and treatment region respectively, which also implies that the assumption of overlap
(Equation~2 in \citet{hahn2017bayesian}) is violated.
...
We also found that averaging the treatment effect along the border has surprising pitfalls.
...
One limitation of our approach to GeoRDDs is the reliance on modeling assumptions.
...
The assumption of equal measurement variance and covariance parameters in the two areas can be relaxed,
by separately tuning the parameters within each area.
...
The GP model would need to be augmented with a temporal component, for example with an anisotropic squared exponential covariance function.
