...
\subsection{The geographic regression discontinuity design}
...
\subsection{Estimating the effect of school quality on house prices}

There has been a long-standing interest in the economics literature in estimating the implicit price of school quality by measuring its effect on house prices.
The theoretical underpinning of this literature is the hedonic valuation model \citep{rosen1974hedonic,sheppard1999hedonic}, and often takes the form of a linear model on the logarithm of the price $p$ of a house with characteristics $x(\svec)$ in a location $\svec$
\begin{equation}
    \log p = s(\svec)\beta + x(\svec)\gamma + g(\svec) + \epsilon
    \,,
    \label{eq:hedonic}
\end{equation}
where $s(\svec_i)$ is the expected quality, reputation, or funding (depending on the focus of the research) of the schools that local residents can access, $g(\svec)$ represents the effect of unobserved covariates that influence house prices in a spatially correlated manner (for example proximity to a good public transportation link or to a popular park), and $\epsilon$ captures the additional unobserved factors that affected the sale prices of this house.
The estimation of $\beta$, the effect of a unit of school quality on the price of a house, is made difficult by the unobserved  $g$, and so ordinary least squares (OLS) estimates are confounded by factors that 

\cite{black2011housing} review the existing research that attempts to value school quality through house prices, and classify the various identification strategies and methodologies that have been used to estimate $\beta$.
Most germane to this paper are the contributions of \citet{black1999better}, who is the first to deploy the regression discontinuity design (RDD) in this context, and \citet{gibbons2013valuing} and \citet{fack2010better}, who develop matching methodologies to address the issue of spatial confounding.
...
One restriction of the hedonic model \autoref{eq:hedonic} is the assumption that the effect size $\beta$ is constant.
It also models the effect of the school district as a linear function of the school quality observable covariates $s(\svec)$.
To allow for heterogenous effects, 
...
\begin{table}
    ...
    \caption{
    \label{table:NYC_ate}
Average difference in log price per square foot between school districts 19 and 27. For each LATE estimand, we show the mean and standard deviation of its posterior distribution, and the tail probability \(\prob(\tau > 0 \mid \Yvec, \hat{\beta}, \hyperparam)\). 
