...
It does not specify a prior directly on the treatment effect long the border;
instead our GP model implicitly gives it a Gaussian process (GP) prior with double 
the covariance function placed on the outcomes on either side of the border.
...
control variates (spatial covariates in GeoRDDs) to estimate the propensity score is inapplicable:
the propensity score for GeoRDD is known to be 0 or 1 in
the control and treatment region respectively, and the assumption of overlap
(Equation~2 in \citet{hahn2017bayesian}) is violated.
We hope that our work will encourage further exploration of Bayesian nonparametric
model specifications for GeoRDDs,
including direct priors for the treatment effect which would address concerns about RIC.
