...
In the paper, we presented and characterized four choices of average treatment effect (ATE) estimands (and corresponding estimators): the uniformly-weighted $\unifavg$, population-weighted $\taurho$, inverse-weighted $\invvar$, and finite population projected $\tauproj$.
...
We illustrate all of our ATE estimators with a simulation.
...
In our main paper, we present the calibrated inverse-variance test, which targets the weak null hypothesis of zero ATE.
It can be generalized to any other choice of ATE estimand defined as a weighted mean over the border, as in \autoref{M-eq:weighted_estimand}.
...
\begin{figure}[!tb]
    \centering
    \includegraphics[width=0.97\textwidth,height=0.8\textheight,keepaspectratio]{../NYC/NYC_plots/pairwise_mean_se.png}
...
\label{sec:tables}
    ...
    \begin{table}[!h]
        ...
        \caption{
            ...
            Posterior mean averaged over 10,000 simulations, posterior standard deviation and true value for each average treatment effect estimand as the wiggliness of the border is increased in the simulations of \autoref{sec:wiggly_border}.
