...
In the Supplementary Materials, we discuss two further choices, the projected land ATE \(\taugeo\), and the projected superpopulation ATE \(\taupop\).
We also provide a simulation study to better understand the characteristics of the different ATE choices.
...
The uniformly weighted ATE \(\unifavg\) is estimated by averaging the entries of the mean posterior at the sentinels.
...
\begin{table}[tbp]
    ...
    \begin{tabular}{llllll}
        ...
        \(\taugeo\)  & Proj. land ATE        & Robust    & Proj. Grid  & Geography   & High     \\
        \(\taupop\)  & Proj. superpop. ATE   & Robust    & Proj. Grid  & Population  & Low \\
        \hline
    \end{tabular}
    \egroup
    \caption{
...
Meanwhile, the weak null only requires the ATE to be zero.
...
As we saw in \autoref{sec:ate}, the ATE estimand can be defined in multiple ways.
...
We derive the analytical calibration of hypothesis tests based on any ATE estimand in \autoref{sec:calibration}.
...
The calibrated inverse-variance test “targets” the weak null hypothesis in the sense that the test statistic is an estimate of the ATE, and thus the test is sensitive to deviations of the ATE from zero, rather than its \(p\)-value being derived directly under the weak null (such as the classical \(t\)-test).
