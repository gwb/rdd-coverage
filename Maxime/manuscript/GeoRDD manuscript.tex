...
\item
  ...
  \item
    so often end up projecting onto distance from border
  \end{itemize}
\item
...
\item
  ...
  \item
    ...
    \item
      extrapolate to \textbf{border curve} \(\border\)
    \item
      take difference to obtain \(\tau(\border): \mathbb{R} \rightarrow \mathbb{R}\) {[}help with notation? or is this too mathematical anyway?{]}
    \end{enumerate}
  \end{itemize}
\item
...
\end{itemize}
    	...
    	A particularly intriguing aspect of GeoRDDs is the functional estimand defined on a one-dimensional manifold embedded in 2D space.
...
The geometry of GeoRDDs makes the very reasonable idea of integrating the treatment effect linearly along the border quite unsuitable.
...
But it lacks an easily interpretable estimand, motivated as it is by the properties of its posterior estimator.
...
We have not yet explored this avenue of research.
    	...
    	Datasets that allow for a GeoRDD approach are numerous, but typically tools from classical univariate RDDs (local linear regression), or from causal inference (matching) have been borrowed for this purpose.
In particular, the direct use of methods developed for univariate RDDs, treating distance from the border as the forcing variable, is asymptotically biased in simple examples.
Our aim was to recognize the importance of the geographical aspect of the problem, and therefore draw from the spatial statistics literature, which brings a rich set of tools designed specifically to model and exploit spatial correlations to obtain more powerful inference.
We therefore used Gaussian process regression, known as kriging in the spatial statistics literature, as the foundation of our GeoRDD method.
