...
Analogously, we focus on the treatment effect at the boundary \(\boundary\) between the treatment and control regions, as defined in \cite{imbens2011regression} and \cite{keeleoverview}.

\begin{equation}
\tau: \boundary \rightarrow \mathbb{R},\,\tau(\sentinels) =  \E\sbr{Y_{i\treat} - Y_{i\ctrol} \mid \svec_i = \sentinels}
...
\end{figure}
    	...
    	The three tests we developed leverage different aspects of the problem, and target two different null hypotheses. One may wonder how their power compares in the presence of a treatment effect. Considering once more the boundary between Louisiana and Mississippi, we imagine an experiment where the unit of analysis is the county, located at its centroid, as shown in Figure \ref{fig:mississippi_counties}.
...
The results of the tests with significance level \(\alpha=0.05\) are shown in Table~\ref{table:power} for \(\tau=0\) (null hypothesis) and \(\tau=1.5\) .
...
After the calibration, the hypothesis test based on the inverse-variance mean retains higher power to detect the constant treatment effect than the mLL and \(\chi^2\) tests.
...
\item
  ...
  \item
    step back: different philosophies
  \end{itemize}
\item
  Hypothesis testing:
  ...
  \item
    power
  \item
    placebo tests to defend against misspecification
  \end{itemize}
\item
  NYC example
\end{itemize}
...
The main limitation of our approach to GeoRDDs is the reliance on many modeling assumptions.
...
\textbf{Connections to other work/fields}

\textbf{Future work/extensions}
The use of Gaussian process regression to analyse GeoRDDs gives flexibility and extensibility to the method.
This presents many interesting paths for future research.

If the outcomes are binary, proportions, or counts, then binomial or Poisson likelihoods could be substituted for the iid normal likelihood used in this paper.
In that case the estimand is no longer interpretable as a discontinuity in the expectation function of the potential outcomes, as in \eqref{eq:functional_estimand}, but as the difference between two hypothesized latent processes.

Because of the need to extrapolate the fitted processes to the border, GeoRDDs could
