...
\cite{keele2015enhancing} propose an alternative methodology, deploying the matching methodology of \cite{zubizarreta2012using} to match units on opposite sides of the border that are near each other geographically and in other covariates, and then analysing the matched outcomes as if they came from a randomized experiment.
...
The topology of borders complicates the definition and interpretation of estimands for the average treatment effect (ATE), which we address in \autoref{sec:ate}.
We obtain Bayesian estimators for multiple possible ATE estimands and discuss their properties.
...
Focusing on a single border between two school districs, we estimate the treatment effect everywhere along the border, obtain estimates of the ATE, and perform and validate a hypothesis test.
...
We denote by \(\tau(\sentinels)\) the \(\numsent\)-vector with \(r^\mathrm{th}\) entry \(\tau(\sentinel_r)\) of the treatment effect evaluated at the sentinel points.
...
\subsection{Inference of the Treatment Effect}\label{sec:inference}}
...
Once we have obtained the posterior of \(\tau\) \eqref{eq:postvar2gp}, estimating the average treatment effect (ATE) along the border will often be of interest.
...
The question remains: what is the most appropriate choice of weights? We next motivate and consider four possible choices of \(\weightb(\sentinel)\), and explore interpretations, advantages, and drawbacks. A summary of their properties is provided in \autoref{table:estimator_properties}.

\hypertarget{uniform-ate}{%
\subsection{Uniform ATE}
}
The simplest choice is uniform weights \(\weightb(\sentinel)=1\), a seemingly reasonable and unopinionated decision.
We estimate \(\unifavg\), the uniformly weighted mean of \(\tau\), by averaging the entries of the mean posterior at the sentinels.
...
\subsection{Density Weighted ATE}
\label{sec:tau_rho}
}
...
\subsection{Inverse-variance Weighted ATE}
\label{sec:invvar}
}
...
\subsection{Projected Finite-Population ATE}
}
...
Let \(\vicinity{\svec}\) indicate inclusion in the border vicinity: one if \(\dist_{\border}\del{\svec} \le \buffer\) and zero otherwise.
...
It should be noted that this choice only affects the location and density of projected units on the border, the Gaussian process always gives low unit weights \autoref{eq:unit_weights} to units far away from the border.

\hypertarget{ATE-summary}{%
\subsection{Summary}\label{sec:summary}}

The properties of the four ATE definitions proposed in this paper are summarized in \autoref{table:estimator_properties}.
...
Once we have obtained the ``cliff face'' estimate \autoref{eq:postvar2gp} and estimated an average treatment effect, we might also naturally wonder whether we can claim to have detected a significant treatment effect at the border.
...
Meanwhile, the weak null only requires the average treatment effect to be zero.
We focus on a test of the weak null hypothesis here, but also provide two tests of the sharp null hypothesis based on the marginal liklihood and a chi-squared statistic in \autoref*{S-sec:hypothesis_testing} of the Supplementary Materials.
...
As we saw in \autoref{sec:ate}, the ``average'' treatment effect can be defined in multiple ways.
...
We now choose the posterior mean of the inverse-variance ATE \(\mu_{\invvar \mid Y}\) as a test statistic.
For \(b=1,\cdots,B\) iterations, we draw \(Y^{(b)}\) from \(\modnull\),
...
We derive the analytical calibration of the inverse-variance test in \autoref{sec:calibration}.
...
We then filter the sales data, by removing sales 
\begin{flatlist}
\item without a reported sale price,
\item outside of the family homes building class categories (one, two, and three family dwellings),
\item missing the square footage or other covariates,
\item without a location due to failed geocoding,
\item smaller than 100 sq ft, and 
\item outliers with log price per square foot less than 3 or more than 8.
\end{flatlist}
We exclude condos and coops because only very few sales report square footage alongside the price.
We display the resulting dataset of 19,578 sales in \autoref{fig:sales_map}.
...
All ATE estimators from \autoref{sec:ate} applied to this setting are shown in \autoref{table:NYC_ate}.
...
Considering this, we do not observe a flagrant departure from uniformity in \autoref{fig:placebo_invvar}(b), which validates the calibrated inverse-variance test, and confirms the significance of the difference in price at the border between the two districts.
...
We use the inverse-variance weighted ATE to attain high power, but the other ATE estimates of \autoref{sec:ate} could be used similarly.
...
We derive the posterior mean of the linear regression coefficients vector \(\betavec\) for the model specified in \autoref{eq:covariates_model}. \(\Yvec\) is the vector of outcomes formed by concatening the treatment outcomes and the control outcomes.
\begin{equation}
    \begin{split}
        % \Yvec &= \begin{pmatrix}
            % \yt \\
...
We seek to obtain a valid hypothesis test against the null hypothesis of zero treatment effect everywhere along the border by using the inverse-variance weighted ATE estimate obtained in \autoref{sec:invvar} as a test statistic.
...
Weighted mean ATE estimators of the form defined in \autoref{eq:weighted_estimator} are linear transformations of \(\mu_{\sentinels \mid Y}\) and so under \(\modnull\), they are normally distributed with mean zero.
...
\begin{equation}
    \Pr\del{ \abs{\mu_{\tauw \mid Y}} \ge \abs{\mu_{\tauw \mid Y^{obs}}} \mid \modnull} = 2 \Phi\del{ -\frac{\abs{\mu_{\tauw \mid Y^{obs}}}}{\sqrt{\var\del{\mu_{\tauw \mid Y} \mid \modnull}}} }\,.
\end{equation}
