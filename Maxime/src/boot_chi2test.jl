function _predict_raw{M<:MatF64,K<:Kernel,MF<:Mean}(gp::GPE, k::K, m::MF, X::M)
    n = size(X, 2)
    cK = cov(k, X, gp.X)
...
_predict_raw{M<:MatF64}(gp::GPE, X::M) = _predict_raw(gp, gp.k, gp.m, X)

function chistat{K<:Kernel,MF<:Mean}(gpT::GPE, gpC::GPE, k::K, mT::MF, mC::MF, X∂::MatF64)
...
chistat(gpT::GPE, gpC::GPE, X∂::MatF64) = chistat(gpT,gpC,gpT.k,gpT.m,gpC.m,X∂)

function sim_chi_null{K<:Kernel,MF<:Mean}(gpT::GPE, gpC::GPE, gpNull::GPE, k::K, mT::MF,mC::MF,
...
function nsim_chi(gpT::GPE, gpC::GPE, X∂::MatF64, nsim::Int; update_mean::Bool=false)
...
function boot_chi2test(gpT::GPE, gpC::GPE, X∂::MatF64, nsim::Int; update_mean::Bool=false)
